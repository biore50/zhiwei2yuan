...
from backbones.rotary import apply_rot_emb, AxialRotaryEmbedding, RotaryEmbedding
# helpers
...
class TimeSformer(nn.Module):
    ...
    def forward(self, video, mask=None):
        b, _, f, h, w, *_, device, p = *video.shape, video.device, self.patch_size
        ...
        video = rearrange(video, 'b c f (h p1) (w p2) -> b (f h w) (p1 p2 c)', p1 = p, p2 = p)
        _, dim, _ = video.shape
        ...
        y = y.view(b,-1,f,h,w)
        cls_token = x[:, 0]
        return y, cls_token # self.to_out(cls_token)
...
if __name__ == '__main__':
    import torch
    from backbones.atten import TimeSformer

    # model
    cuda = torch.cuda.is_available()
    device = torch.device('cuda' if cuda else 'cpu')

    model = TimeSformer(dim=512,channels=512,image_size=7,patch_size=1,num_frames=16,num_classes=10, depth=3,heads=8,dim_head=64, attn_dropout=0.1,ff_dropout=0.1
    ).to(device)

    video = torch.randn(2, 512, 16, 7, 7).to(device)  # (batch x frames x channels x height x width)
    mask = torch.ones(2, 16).bool().to(device)  # (batch x frame) - use a mask if there are variable length videos in the same batch

    y, cls_token = model(video,mask) # (2, 10)
