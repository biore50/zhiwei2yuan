...
if __name__ == '__main__':
    ...
    batch_size = 1
    # B, T, U, D = 2, 10, 3, 4
    # inputs = (3,80,500)  input_lengths = (1,3)
    inputs = torch.FloatTensor( batch_size,C, T, H, W).to(device)

    # model = BasicBlock3d(inplanes=32,planes=32).to(device)
    # model = Bottleneck3d(inplanes=32, planes=32).to(device)
    model = ResNet3d(depth=50,pretrained=None).to(device)
    output = model.forward(inputs)
    print(output.shape)
    bz,C,seq_len,H,W = output.size()
    outs = torch.flatten(output,1)
    out1 = outs.view(bz,seq_len,-1)
    out2 = outs.view(bz, seq_len, -1)
