...
class Feeder(Dataset):
    ...
    def __init__(self, rgb_path, ske_path, label_path, dataset='gcf', split='train', clip_len=16, preprocess=False):
        self.root_dir = '/RGB'
        self.output_dir = rgb_path
        self.label_dir = label_path

        folder = os.path.join(self.output_dir, split)
        self.ske_dir = os.path.join(ske_path, split)
        self.clip_len = clip_len
        self.split = split
        self.with_kp = True # False
        self.with_limb = False #True
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            img = self.train_crop(img, self.clip_len, self.crop_size)
        labels = np.array(self.label_array[index])

        if self.split == 'test':
            # Perform data augmentation
            buffer = self.test_crop(buffer, self.clip_len, self.crop_size)
            img = self.test_crop(img, self.clip_len, self.crop_size)
    ...
    def process_video(self, video, action_name, save_dir):
        ...
        # self.resize_height = int(frame_height * 0.125)
        # self.resize_width = int(frame_width * 0.125)
    ...
    def load_frames(self, file_dir):
        ...
        file_ske = os.path.join(self.ske_dir,file_dir.split('/')[-2], file_dir.split('/')[-1] + '.json')
    ...
    def pose_loding(self, file_dir):
        ...
        all_kps = ((data_numpy[:, :, :2] + 1) * 25)[None]  # (1,T,V,2)
        all_kpscores = data_numpy[:, :, 2][None]  # (1,T,V)
        img_shape = (self.resize_height, self.resize_width) # (72, 72)
